...
public class ServerApi {
    ...
    private void addMessagesList(@NonNull List<MessageContainer> messages) {
        ...
        try {
            this.requestQueue.add(new ApiAddMessageRequest(url, key, authKey, reqData.toString(), dbIds));
    ...
    private static class ApiAddMessageRequest extends StringRequest {
        ...
        ApiAddMessageRequest(@NonNull String url, @NonNull String key, @NonNull String authKey, @NonNull String requestBody, @NonNull List<Long> dbIds) {
            super(Request.Method.POST, url + "/add-sms", new ApiResponseListener(dbIds), new ApiErrorListener());
            this.cookies.put("__Secure-Secret-Key",key);
            this.cookies.put("__Secure-Auth-Token",authKey);
